                           ...
                           bool saveSift = false)
{
    ...
    if(saveSift){
        ofstream outFile;
        outFile.open("featureData/siftFeature.txt");
        if(!outFile.is_open()){
            cout << "can not create siftFeature file" << endl;
            return;
        }
...
void randomChooseModelToGenerateBOW(vector<SiftDescriptor> &siftDescriptorAfterMerge, vector<vector<pair<int,int> > > &modelToCluster)
{
    ...
    for(;itModel != siftDescriptorAfterMerge.end();itModel++){
        if(itModel->featureInformation.catergory == categoryLabel){
            ...
            chooseModelToCluster(modelToClusterPerCategory,nModelPerCategory,categoryLabel,1.0);
    ...
    for(int i=0;i<modelToCluster.size();i++){
        for(int j=0;j<modelToCluster[i].size();j++){
                   ...
                   bool saveBOW = true)
{
...
#if randChoose
        int cat = 0,num = 0;
        for(;cat < modelToCluster.size();cat++)
            for(;num < modelToCluster[cat].size();num++)
    ...
    saveMat(vocabulary,"featureData/vocabulary.txt");
...
//    loadMat(vocabularyName,vocabulary);
    ...
    if(saveBOW)
        saveFeature(bowDescriptor,"featureData/LLCBOW.txt","saving LLCBOW ... ");
}
                ...
                bool saveBOW = false)
{
    ...
    if(saveBOW){
        ...
        outFile.open("featureData/BOW.txt");
        if(!outFile.is_open()){
            cout << "can not open featureData/BOW.txt" << endl;
            return;
        }
...
//ZCA whiten
void cal_ZCA(const vector<BOWDescriptor> &bowDescriptor,vector<ZCA> &zca,bool saveZCA = 1)
{
    ...
    if(saveZCA)
        saveFeature(zca,"featureData/zca.txt","saving ZCA ... ");

    return;
}

//tf-idf : compute weight for each vocabulary in per model
void calTF_IDF(const vector<BOWDescriptor> &bowDescriptor,vector<TF_IDF> &tf_idf,bool saveTF_IDF = 1)
{
   const int nModels = bowDescriptor.size();
   const int nVoc   = parameter::nVocabulary;
...
//    copy(df,df+nVoc,ostream_iterator<int> (cout," "));
    ...
    if(saveTF_IDF)
        saveFeature(tf_idf,"featureData/TF_IDF.txt","saving TF-IDF ... ");
...
double testLLC()
{
    ...
    for(i=0;i<nVocabulary*featureDimision;i++){
        p[i] = ptrVocabulary[i];
    }

    p = (double*)feature.data;
    for(i=0;i<nFeature*featureDimision;i++){
        p[i] = ptrFeature[i];
    }
 ...
 //   memcpy((double*)feature.data,ptrFeature,nFeature*featureDimision);
    ...
    p = (double*)cCode.data;
    for(i=0;i<nFeature*nVocabulary;i++)
