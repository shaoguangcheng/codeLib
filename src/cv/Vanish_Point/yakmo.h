...
//  $Id: yakmo.h 1220 2012-12-25 18:05:59Z ynaga $
// Copyright (c) 2012 Naoki Yoshinaga <ynaga@tkl.iis.u-tokyo.ac.jp>
...
static const  char*  yakmo_short_options = "t:c:k:m:i:nO:v:h";
...
#pragma pack()
    ...
    public:
        centroid_t (point_t& p, const uint nf, const bool delegate = false) :
            delta (0), next_d (0), _norm (p.norm ()), _nelm (0), _nf (nf), _size (0), _dv (0), _sum (0), _body (0) {
            if (delegate) {
                _size = p.size ();
        ...
        // FIXME: don't support sparse format
        void get(std::vector<fl_t> &cents) const {
            for(uint i=0; i<_size; ++i) {
                cents.push_back(_body[i].val);
            }
        }
    ...
    static point_t read_point (char* const ex, const char* const ex_end,
                               std::vector <node_t>& tmp, const bool normalize = false)
    {
        tmp.clear ();
        fl_t norm = 0;
        char* p = ex;
    ...
    void set_point( int n_pts, int n_dim, fl_t *dat,
                    const bool normalize = false)
    {
        ...
        // convert data and add to _point
        for(i=0; i<n_pts; i++) {
            // clear old data
            _body.clear();
            ...
            // insert each dimension
            for(j=0; j<n_dim; j++) {
                fi = j+1;
                fv = dat[i*n_dim + j];
                norm += fv*fv;
                _body.push_back(node_t(static_cast<uint>(fi), fv));
            }

            // normalize
            if (normalize) {
                norm = std::sqrt (norm);
                for (std::vector <node_t>::iterator it = _body.begin ();
                     it != _body.end (); ++it)
                    it->val /= norm;
                norm = 1.0;
            }
            ...
            // insert points
            _point.push_back( point_t(&_body[0], static_cast <uint> (_body.size ()), norm) );
    ...
    void set_point( int n_dim, std::vector<fl_t> &dat,
                    const bool normalize = false )
    {
        int     i, j;
        fl_t    norm = 0;
        ...
        typedef std::vector<fl_t>::iterator     it_dat;
        it_dat  it1;
        ...
        // insert all data to _point
        for(i=1, j=0, it1=dat.begin(); it1!=dat.end(); ++it1, i++) {
            fi = ++j;
            fv = *it1;
            norm += fv*fv;
            _body.push_back(node_t(static_cast<uint>(fi), fv));
    ...
    void init () {
        struct rng_t {
            ...
            std::tr1::variate_generator <std::tr1::mt19937,
                    std::tr1::uniform_real <> > gen;
            rng_t () : gen (std::tr1::mt19937 (), std::tr1::uniform_real <> (0, 1)) {}
        ...
        //
#ifdef USE_HASH
        std::tr1::unordered_set <uint> chosen;
#else
        std::set <uint> chosen;
#endif
...
public:
    ...
    void train_from_data(int n_pts, int n_dim, fl_t *dat,
                         const uint iter,
                         const uint output = 0,
                         const bool test_on_other_data = false,
                         const bool instant = false)
    {
        ...
        // set input data
        km->set_point (n_pts, n_dim, dat, _opt.normalize);
    ...
    void train_from_data(int n_dim, std::vector<fl_t> &dat,
                         const uint iter,
                         const uint output = 0,
                         const bool test_on_other_data = false,
                         const bool instant = false)
    {
        ...
        // set input data
        km->set_point (n_dim, dat, _opt.normalize);
    ...
    void get_cents(std::vector<fl_t> &cents, int &m) {
        // clear old data
        cents.clear();

        // get all centroids
        for(uint i=0; i<_kms.size(); ++i) {
            const std::vector <kmeans::centroid_t> &centroids = _kms[i]->centroid();

            for(uint j=0; j<centroids.size(); ++j) {
                centroids[j].get(cents);
                //centroids[j].print(fp, j);
            }
        }
