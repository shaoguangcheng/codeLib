...
SPixelPosition CRecognition::findSegment(CRawImage* image)
{
    ...
    //draw circle and marker in the center
    Mat* img;
    img = image->to_mat();
...
//    printf("maxsize = %d\n,num = %d\n",maxSize,num);
    ...
    line(*img,Point(result.x-5,result.y),Point(result.x+5,result.y),Scalar(0,0,0),2,8,0);
    line(*img,Point(result.x,result.y-5),Point(result.x,result.y+5),Scalar(0,0,0),2,8,0);
